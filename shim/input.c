...
static void mouse_send(Mouse *m) {
  Mouse drop;
...
static void pointer_motion(void *data, struct wl_pointer *pointer,
  ...
  if (debug_input) {
    fprint(2, "wayland-input: motion %d,%d buttons=%d\n",
           wl_state->mouse_state.xy.x, wl_state->mouse_state.xy.y,
...
static void pointer_button(void *data, struct wl_pointer *pointer,
  ...
  if (debug_input) {
    fprint(2, "wayland-input: button %u state=%u p9=%d mask=%d\n", button,
           state, plan9_button, wl_state->mouse_state.buttons);
  }
  mouse_send(&wl_state->mouse_state);
}
...
static void pointer_axis(void *data, struct wl_pointer *pointer, uint32_t time,
  ...
  /* plan 9 buttons: 8 = scroll up, 16 = scroll down */
  ...
  wl_state->mouse_state.buttons |= scroll_button;
  wl_state->mouse_state.msec = time;
  mouse_send(&wl_state->mouse_state);
  ...
  if (debug_input) {
    fprint(2, "wayland-input: axis %u value=%g scroll_button=%u\n", axis,
           wl_fixed_to_double(value), scroll_button);
  }
}
...
static void keyboard_enter(void *data, struct wl_keyboard *keyboard,
  ...
  if (wl_state)
    wl_state->kb_enter_serial = serial;
}
...
static void keyboard_key(void *data, struct wl_keyboard *keyboard,
  ...
  /* handle Ctrl modifier → Acme keybinds */
  if (xkb_state && xkb_state_mod_name_is_active(xkb_state, XKB_MOD_NAME_CTRL,
                                                XKB_STATE_MODS_EFFECTIVE)) {
    if (keysym >= XKB_KEY_a && keysym <= XKB_KEY_z) {
      /* Ctrl+letter: Acme uses Kcmd for copy/cut/paste/undo */
      switch (keysym) {
      case XKB_KEY_c:
        rune = Kcmd + 'c';
        goto send;
      ...
      case XKB_KEY_z:
        ...
        goto send;
      default:
        /* standard control chars: ^A=0x01, ^E=0x05, etc. */
        rune = keysym - XKB_KEY_a + 1;
        goto send;
      }
    }
    if (keysym >= XKB_KEY_A && keysym <= XKB_KEY_Z) {
      /* Ctrl+Shift+letter → Kcmd+uppercase (e.g. redo) */
      rune = Kcmd + keysym - XKB_KEY_A + 'A';
      goto send;
    }
  }
  ...
  if (keysym >= XKB_KEY_F1 && keysym <= XKB_KEY_F12) {
    ...
    rune = Kesc;
  } else if (keysym == XKB_KEY_Insert || keysym == XKB_KEY_KP_Insert) {
    rune = Kins;
  } else if (keysym == XKB_KEY_ISO_Left_Tab) {
    rune = '\t';
  } else if (keysym == XKB_KEY_KP_Enter) {
    rune = '\n';
  } else if (keysym == XKB_KEY_KP_Left) {
    rune = Kleft;
  } else if (keysym == XKB_KEY_KP_Right) {
    rune = Kright;
  } else if (keysym == XKB_KEY_KP_Up) {
    rune = Kup;
  } else if (keysym == XKB_KEY_KP_Down) {
    rune = Kdown;
  } else if (keysym == XKB_KEY_KP_Home) {
    rune = Khome;
  } else if (keysym == XKB_KEY_KP_End) {
    rune = Kend;
  } else if (keysym == XKB_KEY_KP_Page_Up) {
    rune = Kpgup;
  } else if (keysym == XKB_KEY_KP_Page_Down) {
    rune = Kpgdown;
  } else if (keysym == XKB_KEY_KP_Delete) {
    rune = Kdel;
  }
