...
int wayland_init(char *label, int width, int height) {
  ...
  wl_state->clipreq = chancreate(sizeof(ClipCmd), 4);
  ...
  wl_state->xdg_surface =
      xdg_wm_base_get_xdg_surface(wl_state->wm_base, wl_state->surface);
  ...
  xdg_toplevel_add_listener(wl_state->xdg_toplevel, &xdg_toplevel_listener,
                            NULL);
  xdg_toplevel_set_title(wl_state->xdg_toplevel, label);
  xdg_toplevel_set_app_id(wl_state->xdg_toplevel, "wave");
  wl_surface_commit(wl_state->surface);

  wl_display_roundtrip(wl_state->wl_display);

  wl_state->wld_ctx =
      wld_wayland_create_context(wl_state->wl_display, WLD_SHM, WLD_NONE);
  if (!wl_state->wld_ctx) {
  ...
  if (debug_wayland && getenv("ACME_WAYLAND_CLEAR") != nil) {
    if (wl_state->wld_buf && wl_state->wl_buf &&
      ...
      wld_fill_rectangle(wl_state->renderer, color, 0, 0, wl_state->width,
                         wl_state->height);
      wld_flush(wl_state->renderer);
      wl_surface_damage(wl_state->surface, 0, 0, wl_state->width,
                        wl_state->height);
...
int wayland_resize_surface(int width, int height) {
  ...
  if (wl_state->wld_buf && wl_state->wld_buf->width == (uint32_t)width &&
