...
/* Global variable definitions (declared extern in dat.h) */
/* Note: display, screen, font, home are provided by Plan 9 libraries */
uint		globalincref;
uint		seq;
uint		maxtab;
Mouse		*mouse;
...
iconinit(void)
{
	...
	tagcols[BORD] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, ACME_TAG_BORD);
	tagcols[TEXT] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, ACME_TAG_TEXT);
	tagcols[HTEXT] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, ACME_TAG_HTEXT);
	...
	dividercolor = allocimage(display, Rect(0,0,1,1), screen->chan, 1, ACME_DIVIDER);
	backgroundcolor = allocimage(display, Rect(0,0,1,1), screen->chan, 1, ACME_BACKGROUND);
	...
	r = insetrect(r, 2);
	fillellipse(modbutton, (Point){r.min.x + 3, r.min.y + 5}, 3, 3, textcols[TEXT], ZP);

	r = button->r;
	colbutton = allocimage(display, Rect(0,0,1,1), screen->chan, 1, ACME_COLBUTTON);
	but2col = allocimage(display, r, screen->chan, 1, ACME_BUTTON2);
	but3col = allocimage(display, r, screen->chan, 1, ACME_BUTTON3);
}
